...
def conv2d_bn_block(in_channels,
                    out_channels,
                    kernel=3,
                    momentum=0.01,
...
def deconv2d_bn_block(in_channels,
                      out_channels,
                      use_upsample=True,
                      kernel=4,
                      stride=2,
...
class Generator(nn.Module):
    def __init__(self,
        ...
        act = activation
        self.label_d = torch.tensor([0, 1]).half().cuda()
        ...
        self.conv7_d = nn.Sequential(
            conv_block(nf, nf, activation=act),
            conv_block(nf, n_channels, activation=nn.Tanh),
        )

        self.conv7_h = nn.Sequential(
            conv_block(nf, nf, activation=act),
            conv_block(nf, n_channels, activation=nn.Tanh),
        )
...
class Discriminator(nn.Module):
    def __init__(self, n_channels=1, init_filters=32, batch_norm=False):
        ...
        nf = init_filters
        self.label_d = torch.ones(1).long().cuda()
